"""ReasoningChatOpenAI 契约：reasoning_content 增量转 thinking 块、text 增量
转 text 块、块级合并、以及历史回传上游时 thinking 块被剥离。全部离线。"""
...
def test_reasoning_delta_becomes_thinking_block():
    message = convert(make_model(), {"reasoning_content": "先分析"})
    assert message.content == [{"type": "thinking", "thinking": "先分析", "index": 0}]
...
def test_consecutive_deltas_merge_into_single_blocks():
    ...
    assert merged.content == [
        {"type": "thinking", "thinking": "思考A思考B", "index": 0},
        {"type": "text", "text": "正文", "index": 1},
    ]


def test_history_with_thinking_blocks_is_stripped_for_upstream():
    # 含思考的历史消息回传上游时，thinking 块必须被请求侧剥离（智谱对未知
    # 块类型返回 400，已实证）；text 块保留（index 多余键上游接受，已实证 200）
    message = AIMessageChunk(content=[
        {"type": "thinking", "thinking": "秘密思考", "index": 0},
        {"type": "text", "text": "可见正文", "index": 1},
    ])
    payload = _convert_message_to_dict(message)
    assert payload["content"] == [{"type": "text", "text": "可见正文", "index": 1}]
