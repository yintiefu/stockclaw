...
先 import 生产图模块（模块级装配执行一次，验证生产构建路径——含
ReloadableSkillsMiddleware 与过滤 backend 的技能栈），再用注入的脚本化
模型重建；模型额外识别「列出当前技能」，让 E2E 断言实际注入的技能视图。
浏览器夹具绝不调用真实行情源；MCP 走共享的 stdio 假服务。
"""
...
graph = asyncio.run(production_graph.build_graph(
    model=SkillsAwareScriptedModel([
