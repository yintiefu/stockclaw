...
function ToolFallbackTrigger({
  ...
  const label = isCancelled ? "已取消的工具" : "调用工具";
...
function ToolFallbackError({
  ...
  const headerText = isCancelled ? "取消原因:" : "执行错误:";
...
const approvalOptionLabel = (option: ToolApprovalOption) =>
  option.label ??
  (hasOwn(APPROVAL_OPTION_DEFAULT_LABELS, option.kind)
...
function ToolFallbackApproval({
  ...
  const options = declaredOptions?.filter((o) =>
    hasOwn(APPROVAL_OPTION_DEFAULT_LABELS, o.kind),
  );
