...
- 新增「技能管理」页（`/skills`）：区分只读内置技能与用户导入技能；支持从本地
  文件夹或 ZIP 导入（导入默认停用、同名不覆盖）、启用/停用、查看 Markdown 详情与
  永久删除。启停由活动根 `skills.path` 与同级 `<name>.disabled` 停用根的目录位置表达。
- 导入边界：请求体上限 36 MiB、最多 256 个文件、解包总量 25 MiB（流式计数、不信任
  ZIP 声明大小）；拒绝加密/符号链接成员、路径穿越与 macOS 噪音；失败不留临时目录。
- Agent 视图隔离：内置、无效、与内置同名冲突、停用的技能都不能从错误命名空间列出
  或读取；技能后端在每次 `ls` / `read_file` / 下载上强制授权，阻止 `..` 逃逸与
  符号链接穿透；用户技能永不进入「问 AI」与四个固定工作流。
- 显式刷新：已有 α-mind 会话保持启动时的技能缓存，输入精确 `/reload-skills` 才重新
  加载（只扫描目录并更新会话状态，不调用模型、不执行工具），回复确定性可见计数。
- 待审批（HITL）期间 α-mind 输入框与发送按钮禁用，审批面板仍是唯一恢复入口。
- 边界说明：启用用户技能后，其元数据进入本地检查点，读取的指令内容会发送给所配置
  的模型；技能管理只做本地磁盘操作，不提供 Git 导入、编辑器、市场或执行能力。
