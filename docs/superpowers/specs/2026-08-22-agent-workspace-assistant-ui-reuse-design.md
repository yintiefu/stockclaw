...
1. `ThreadProps` 增加两个 **ReactNode 元素 props**（评审修订：不走 `ThreadComponents` 组件类型槽）——`composer?: ReactNode`（缺省回落 demo 内置 Composer，工作台传入带禁用态 / SteerAway 换装的元素）、`footerExtra?: ReactNode`（渲染在 ViewportFooter 内 Composer 之前）。理由：工作台侧的 Composer/FooterExtra 必然携带闭包状态（pendingApproval、statusNote、activeThread 等），只能以内联函数提供组件类型——函数身份随流式重渲染变化，React 会按"不同组件类型"卸载重挂子树（输入焦点丢失、SteerAway 草稿清空、工具折叠状态抖动）；元素按类型协调天然免疫。`ThreadComponents` 保持 demo 原样；`ToolFallback` 槽用模块级静态组件实现，`onOpenArtifact` 经 React Context 注入。
...
3. 文案中文化（含 sr-only 与 aria-label——它们是屏幕阅读器可感知文案）：composer placeholder（「输入投研问题」）、Welcome 欢迎语（投研语境）、tooltips（复制 / 刷新 / 编辑 / 更多 / 导出 Markdown / 滚动到底 / 发送 / 停止 / 上一个 / 下一个）、thread.tsx skeleton 的 sr-only「Loading conversation」→「正在加载会话」、EditComposer 的 Cancel/Update →「取消 / 更新」、ToolFallback 的「调用工具 / 已取消」及其子组件文案「Result:」→「执行结果：」「Error:」→「执行错误：」「Cancelled reason:」→「取消原因：」、ToolFallbackApproval 的 Allow/Deny 系文案（AG-UI 下预期不渲染，见 §8 验证点，中文化以备）、ToolGroup 的「N 次工具调用」、Reasoning 的「思考过程」、markdown-text 代码头「Copy」→「复制」、image.tsx 的 aria-label「Download image / Copy image」→「下载图片 / 复制图片」与 sr-only「Generating image…」→「正在生成图片…」、attachment.tsx 的 DialogTitle sr-only「Image Attachment Preview」→「图片附件预览」（该组件当前无 adapter 不渲染，一并处理）。`aui-*` 类名与 data-slot 保持不变。实施收尾用 `grep` 扫描复制件内剩余用户可见英文字符串查漏。
