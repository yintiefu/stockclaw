...
- 维护连接健康、活跃 lease、异步关闭和 stale session 淘汰。
- 原始 secret 只存在 Registry 的连接对象内。

`CapabilityResolver`：

- 从 Coordinator preview 返回的权威 `selected_skills` 解析 Skill generation。
- 组合内置工具、两个 Skill 工具和全局启用的 MCP 工具。
- 构建动态 MCP 审批策略。
- 返回 `CapabilityLease`，不修改 thread/run 文档。

`CapabilityLease`：

- 持有 Skill runtime snapshot、MCP tool bindings、policy middleware 和 generation 引用。
...
- MCP adapter interceptor 的结果和异常脱敏。
- health 错误脱敏。
- tool catalog 描述脱敏。
- 关闭连接时清除内存引用。
...
MCP session 按 server 缓存。Registry 使用 `AsyncExitStack` 管理官方 adapter 的 session context：
...
6. 应用脱敏 interceptor、server 串行锁和 alias。
7. 缓存 session、tools、health 和 config revision。

`test` 完成 initialize 和基础能力检查。`refresh` 还会更新工具目录。连接失败写入脱敏且最多 500 字符的 health message；成功写入 checked time 和 tool count。health 更新属于配置文档修改，会递增 revision。

对应 server 的 connection-affecting 配置改变、连接 transport error、工具调用期间取消或后端 shutdown 时，连接进入 stale 并关闭。若有 active capability lease，相关配置修改、test 和 refresh 返回 `MCP_CONFIG_BUSY`，不热切换活跃 Graph 的工具对象。
...
Stop 或断连取消 Graph 时，尚未完成的 MCP 结果不得进入 Graph、SSE 或 RunJournal。调用中的 server session 标记 stale，关闭完成后才能重新连接。
...
纯 resume 复用 active handle 内的同一 lease、tools、middleware 和 MemorySaver。resume 不重新读取 `mcp.json` 或 Skill 文件。
...
使用锁定版 `HumanInTheLoopMiddleware(interrupt_on=...)`。`interrupt_on` 只包含 MCP alias，每项只允许 `approve` 和 `reject`，不开放 edit/respond。
...
4. 生成按原始顺序排列的 LangChain decisions。
5. 使用现有 snapshot 重建 request-scoped Graph。
6. 将同一产品 run 持久化回 `running` 并追加 protocol run ID。
7. 为 `approve + thread_session` 写入内存 allowance。
8. 清空 pending 并恢复流。
...
页面从 REST history 恢复 pending message 时，只有 thread `last_run.status == "awaiting_approval"` 且 `resume_available=true`，才把 `metadata.custom["ag-ui"].interrupts` 注入 assistant-ui。
...
所有 API 继续受现有 `/api/*` 可选 `VR_API_KEY` middleware 和 CORS 策略保护。REST response 使用 snake_case；AG-UI 标准字段继续按锁定 runtime 的 camelCase 格式。
...
- 保留真实 `/api/kline`、`/api/finance`，以及直接 `Quotes.F10` smoke 为 `live` 发布前检查；不新增 F10 HTTP route。
...
- session cache、stale eviction、取消和 lifespan shutdown。
