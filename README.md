...
│   ├── tools.py         AI 工具层（25 个数据工具，全部 Graph 与 MCP 共用）
...
- **Skills 分内置与用户两类**：内置技能随仓库只读（始终启用）；用户技能在「设置 → 技能管理」页
  从本地文件夹或 ZIP 导入，**默认停用**（落盘时自动在 frontmatter 注入 `enabled: false`）。
  启用/停用由 SKILL.md frontmatter 的 `enabled: true|false` 字段表达（缺省视为 `true`），
  切换原地改写该行、技能目录不移动；旧版同级 `skills.path.disabled` 停用根仅作遗留读取，
  从中启用会一次性迁移到活动根。
  导入同名不覆盖，文件夹/ZIP 有 36 MiB 请求体、256 文件与 25 MiB 解包上限。
- **新会话读当前配置，旧会话要显式刷新**：新会话自动使用当前启用的技能；已有会话保持
  启动时的缓存，在 α-mind 输入精确的 `/reload-skills` 才重新加载（不调用模型）。
- **用户技能只进 α-mind**：页面内「问 AI」与四个固定工作流（辩论/反思/复盘/提炼）只读
  内置技能，永不读取用户导入的技能。启用用户技能后，其元数据会写入本地会话检查点，
  读取的指令内容会发送给你配置的模型。
- **技能文件系统只读且限制在技能根**：模型经 `ls` / `read_file` 渐进读取技能（先见
  名称与描述，按需读全文）；无效、与内置同名冲突、停用的技能不出现在 Agent 视图，
  路径逃逸（`..` 越出技能目录）与符号链接穿透都会被拒绝。
